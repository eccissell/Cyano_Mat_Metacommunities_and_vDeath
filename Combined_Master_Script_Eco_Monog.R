...
library(dplyr)
library(matrixStats)
#Broken into sections corresponding to separate analyses presented in manuscript
########
##Metacommunity benthic cover
########
#Read in data
benthic=read.csv('/Users/cissell/Desktop/summer_2019_benthic_cover_data.csv')
#View
View(benthic)
...
df=read.xls('/Users/cissell/Desktop/follows_overall.xlsx')
View(df)
...
########
#Read in data
matfollow=read.xls('/Users/cissell/Desktop/mat_video_follow_counts.xlsx')
#Check the data
View(matfollow)
...
#Excluding massive outlier
sd(matfollow2$total.bites) #81.60 bites SD
mean(matfollow2$total.bites) #66.76 bites Mean
...
########
##Metacommunity model analysis
########
#Set WD
setwd("/Users/cissell/Documents/Manuscripts/Microbial Landscapes/Model/Output5/")

#Read in files and add in file name as a column 
files = list.files()
data2=lapply(files, read.table, header=FALSE, sep=" ")
for (i in 1:length(data2)){data2[[i]]<-cbind(data2[[i]],files[i])}
mat_metcom <- do.call("rbind", data2) 
colnames(mat_metcom)[c(1,2)]<-c("Substrate", "Mat")

#This is adding weird 3rd column, so remove, but check yours to make sure it is adding empty 3rd column before running
mat_metcom = mat_metcom%>%
  select(-V3)

#Now add in Time step column with repeating sequence and group column to distinguish repeat runs within parameterization
mat_metcomtime = mat_metcom%>%
  mutate(Time=rep(1:365,length.out=n()))%>%
  mutate(Run=rep(row_number(),each=365,length.out=n()))

#Split file column into nPred, nGrwth, nDisp
mat_metcomtime[c('nPred','nGrwth','nDisp')] = str_split_fixed(mat_metcomtime$files,'_',3)

#Clean up
mat_metcomsplit=mat_metcomtime%>%
  mutate(nPred = str_remove_all(nPred, "Output"))%>%
  mutate(nDisp = str_remove_all(nDisp, '.txt'))

#Now add in Columns that enumerates the parameterization
mat_metcom_enum = mat_metcomsplit %>%
  mutate(Pred=case_when(endsWith(nPred,'H') ~"0.43",
                        endsWith(nPred,'M') ~"0.23",
                        endsWith(nPred,'L') ~"0.03",
                        endsWith(nPred,'Z') ~"0.00")) %>%
  mutate(Grwth=case_when(endsWith(nGrwth,'H') ~"0.8",
                         endsWith(nGrwth,'M') ~"0.5",
                         endsWith(nGrwth,'L') ~"0.2",
                         endsWith(nGrwth,'Z') ~"0.00")) %>%
  mutate(Disp=case_when(endsWith(nDisp,'H') ~"0.01",
                        endsWith(nDisp,'M') ~"0.001",
                        endsWith(nDisp,'L') ~"0.0001",
                        endsWith(nDisp,'Z') ~"0.00"))

#Now add column that is propBCM
mat_metcom_prop = mat_metcom_enum%>%
  mutate(propBCM=Mat/(Mat+Substrate))

#Create separate DF that is just change across 365 days for each model run
mat_metcom_deltaprop=mat_metcom_prop%>%
  subset(Time=="1" | Time=="365") %>%
  group_by(Run) %>%
  mutate(deltprop =( propBCM[Time == "365"] - propBCM[Time=="1"] )/ propBCM[Time=="1"]) %>%
  ungroup()%>%
  subset(Time=="365")

#Change to numeric
mat_metcom_deltaprop$Pred=as.numeric(mat_metcom_deltaprop$Pred)
mat_metcom_deltaprop$Grwth=as.numeric(mat_metcom_deltaprop$Grwth)
mat_metcom_deltaprop$Disp=as.numeric(mat_metcom_deltaprop$Disp)
mat_metcom_deltaprop$propBCM=as.numeric(mat_metcom_deltaprop$propBCM)

#Create mean prop among model replicates for better visualization
mean_gconst=mat_metcom_prop%>%
  group_by(across(3:4))%>%
  mutate(meanprop=mean(propBCM))%>%
  mutate(stdev=sd(propBCM))%>%
  ungroup()%>%
  mutate(ymin=meanprop-stdev)%>%
  mutate(ymax=meanprop+stdev)

mean_gconst$nPred=factor(mean_gconst$nPred,
                         levels = c("Z","L","M","H"))
mean_gconst$nGrwth=factor(mean_gconst$nGrwth,
                          levels = c("Z","L","M","H"))
mean_gconst$nDisp=factor(mean_gconst$nDisp,
                         levels = c("H","M","L","Z"))
main_fig_model=ggplot(aes(x=Time,
                          y=meanprop,
                          color=nDisp),
                      data=mean_gconst) +
  geom_line(alpha=0.8,size=1)+
  #  geom_ribbon(alpha=0.4,aes(ymin=ymin,
  #                            ymax=ymax))+
  theme_classic()+
  facet_grid(vars(nPred),vars(nGrwth)) +
  ylab("Proportional BCM cover")+
  xlab("Time (days)")+
  scale_color_manual(name="Dispersal Prob.",values=c("#440154",
                                                     "#365c8d",
                                                     "#1fa187",
                                                     "#a0da39"))
main_fig_model
ggsave("/Users/cissell/Desktop/metcom_model.svg",width=6.5,height=8.5,units="in",main_fig_model)
