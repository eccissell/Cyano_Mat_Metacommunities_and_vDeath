...
#DATE LAST CODE UPDATED: AUGUST 23, 2025
#####################################################
#SECTION 1
...
depthlm = lm (BCM~Depth, data = benthos_wide)
summary(depthlm)
########
...
#Read in df
df <- read.csv("/Users/cissell/Documents/Manuscripts/BCM_Landscapes_and_Model/DATASETS/coring_experiment_df.csv", stringsAsFactors = FALSE)
#Clean up df and prepare 'status' for KM analysis
df$Treatment <- factor(df$Treatment, levels = c("H","W"))
df$status    <- as.integer(toupper(df$Died) == "YES")      # 1 = died, 0 = censored

# Run Kaplan–Meier by treatment
kmfit <- survfit(Surv(Duration, status) ~ Treatment, data = df)
summary(kmfit)

#Log-rank p-value 
lr   <- survdiff(Surv(Duration, status) ~ Treatment, data = df)
...
#Save plot
ggsave("/Users/cissell/Desktop/decay_curve.svg",width=6.5,height=4.5,units="in",decay_km_plot)

#Test for difference in survivability at median survival for unmanipulated treatment (i.e., natural expected survivability)
#Extract median
kmW  <- survfit(Surv(Duration, status) ~ 1, data = subset(df, Treatment == "W"))
...
km   <- survfit(Surv(Duration, status) ~ Treatment, data = df)
